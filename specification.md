...
large prime subgroup $G$ with order $r$ generated by the base point $B$, an
EC-VRF is VRF-AD where $pk = sk \cdot g$ and VRF $Sign$ is based on an elliptic
...
In particular *step 5* of section 5.4.3 is defined as:
...
2. $k \leftarrow Nonce(sk, input)$ (see [RFC9381] section 5.4.2)
3. $c \leftarrow Challenge(Y, H, preout, k \cdot B, k \cdot input)$ (see [RFC9381] section 5.4.3)
...
3. $c' = Challenge(pk, input, output, U, V)$ (see [RFC9381] Section 5.4.3)
