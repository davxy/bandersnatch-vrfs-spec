...
with Bandersnatch [MSZ21] [@MSZ21], an elliptic curve constructed over the
...
with large subgroup $\G$ of prime order $r$ generated by the base
point $G$, we define a VRF-AD, called *EC-VRF*, in which $pk = sk \cdot G$ and VRF $Sign$ is based 
on an elliptic curve signature scheme defined in Section [3.2](#3.2. Sign).
